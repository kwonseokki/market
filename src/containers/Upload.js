...
import { PageLock } from "../components";
const Upload = ({ onChange, onSubmit, selecteImage }) => {
  const user = useSelector((state) => state.authReducer);
  const { uid } = user;
  if (!uid) return <PageLock />;
  return (
    <div className="item-container container product-upload">
      <form onSubmit={onSubmit}>
        ...
        <button
          type="submit"
          className="text-white focus:outline-none focus:ring-4 font-medium rounded-full text-sm px-5 py-2.5 text-center mr-2 mb-2 dark:focus:ring-yellow-900"
